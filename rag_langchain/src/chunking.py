...
class TableOfContentsChunker:
    ...
    def _sanitize_toc(self, raw_toc: List[Dict], total_pages: int) -> List[Dict[str, int]]:
        """
    ...
    def split_documents(self, documents: List[Document]) -> List[Document]:
        ...
        if len(documents) == 1:
            ...
            # extract first 5
            assert self.delim is not None, "delim must be provided for single document input."
        ...
        if len(documents) == 1:
            ...
            documents = [Document(page_content=page, metadata={
        ...
        for entry in clean_indices:
            ...
            if len(merged_content) > self.max_chunk_size:
                ...
                final_chunks.append(Document(page_content=merged_content, metadata=base_metadata))

        print(f"Created {len(final_chunks)} chunks from {len(clean_indices)} TOC entries.")
